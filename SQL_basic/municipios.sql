CREATE TABLE municipios(
    ...
    id_estado INT,
    municipio_codigo_ibge VARCHAR(15),
    municipio VARCHAR(40)
);

INSERT INTO municipios(id, id_estado, municipio_codigo_ibge, municipio) VALUES
...
-- Busca municipio por codigo do IBGE
SELECT
	municipio_codigo_ibge,
    municipio,
    abreviacao,
...
INNER JOIN estados e
WHERE m.id_estado = e.id
AND municipio_codigo_ibge = '3113701'
LIMIT 1

-- Busca municipio por nome do municipio
SELECT
	municipio_codigo_ibge,
    municipio,
    abreviacao,
...
INNER JOIN estados e
WHERE m.id_estado = e.id
AND municipio = 'JANDAÍRA'
LIMIT 1

-- Apresenta a quantidade de municipios por estado
SELECT 
	DISTINCT e.estado,
	COUNT(m.id_estado) AS quant_de_municipios_por_estado
FROM municipios m
INNER JOIN estado e
WHERE m.id_estado = e.id
...
-- Apresenta a quantidade de municipios por região
SELECT 
	DISTINCT e.regiao,
	COUNT(m.id_estado) AS quant_de_municipios_por_regiao
FROM municipios m
INNER JOIN estados e
WHERE m.id_estado = e.id
GROUP BY e.regiao
